//
// Created by student on 17.03.18.
//
#include <stdlib.h>
#include <stdio.h>
#include <sys/time.h>
...
int parseArg(size_t *arraySize, size_t *blockSize, int *typeOfAlloc, struct Operations (*operations)[3], int argc, char **argv) {
    ...
    *blockSize = (size_t) strtol(argv[++current], &argDump, 0);
    ...
    for (int i = 0; current < argc; i++, current++) {
        ...
        else if (strcmp("createArray", argv[current]) == 0)
...
int main(int argc, char **argv) {
    ...
    void* library = dlopen("./libmyDynamicLibrary.so", RTLD_LAZY);
    *(void**)(&allocArray) = dlsym(library, "allocArray");
    *(void**)(&freeArray) = dlsym(library, "freeArray");
    *(void**)(&allocBlock) = dlsym(library, "allocBlock");
    *(void**)(&findUsedBlock) = dlsym(library, "findUsedBlock");
    *(void**)(&findUnusedBlock) = dlsym(library, "findUnusedBlock");
    *(void**)(&freeBlock) = dlsym(library, "freeBlock");
    *(void**)(&findMatchingBlock) = dlsym(library, "findMatchingBlock");

    FILE *file;
    file = fopen("./raport3b.txt", "a");
    if (!file) {
